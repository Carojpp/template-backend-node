...
const getUserTelefonoModels = (phone) => {
    console.log('file: userModel.js -> getUserTelefonoModels')
    const result = userDB.filter((user) => user.phone === phone);
    if (result.length===0){
        return `No se encontraron resultados para ${phone}`
    }
}
...
export {
    getUsersModels, getUserEmailModels, getUserTelefonoModels, createUserModels
}